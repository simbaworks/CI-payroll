...
class Commonmodel extends CI_Model {
    ...
    public function generate_temp_table($by_force = FALSE, $survey_code = '0', $survey_id = '0', $no_reply = TRUE) {
        if ($survey_code <> '0' && $survey_id <> '0') {
            ...
            $cond_and = array();
            $join_logic = array();
            $cond_and = array('survey_id' => $survey_id, 'is_roster' => '0');
            $blocks = $this->fetch($p_table = 'blocks', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('blocks.block_order' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
            $cnt = 0;
            foreach ($blocks as $block_row) {
                ...
                $join_logic = array();
                $join_logic[0] = array('first_table' => 'block_questions', 'join_on' => 'block_questions.question_id = questions.id', 'join_type' => 'inner', 'join_select' => array('block_questions.block_id', 'block_questions.question_id'), 'join_condition' => array('block_questions.status' => '1', 'block_questions.block_id' => $block_id));
                $join_logic[1] = array('first_table' => 'blocks', 'join_on' => 'block_questions.block_id = blocks.id', 'join_type' => 'inner', 'join_select' => array('blocks.name as block_name'), 'join_condition' => array('blocks.is_roster' => '0', 'blocks.status' => '1'));
                ...
                $join_logic[3] = array('first_table' => 'question_details', 'join_on' => 'question_details.question_id = questions.id', 'join_type' => 'inner', 'join_select' => array('question_details.question_type_id', 'question_details.question_sum, question_details.is_other_text, question_details.other_text_name, question_details.is_none_of_the_above, question_details.other_text_name, question_details.data_label, question_details.question_code'));
                $join_logic[4] = array('first_table' => 'options', 'join_on' => 'options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('options.id as option_id', 'options.name as option_name'));
                $join_logic[5] = array('first_table' => 'matrix_options', 'join_on' => 'matrix_options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('matrix_options.id as matrix_option_id', 'matrix_options.name as matrix_option_name'));
                $questions = $this->fetch($p_table = 'questions', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('blocks.block_order' => 'ASC', 'block_questions.question_order ' => 'ASC', 'options.option_order' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
                ...
                if (count($parent_child) > 0) {
                    ...
                    foreach ($parent_child as $question_code) {
                        $cond_and = array('status' => '1');
                        $join_logic[0] = array('first_table' => 'block_questions', 'join_on' => 'block_questions.question_id = questions.id', 'join_type' => 'inner', 'join_condition' => array('block_questions.status' => '1'));
                        $join_logic[1] = array('first_table' => 'blocks', 'join_on' => 'block_questions.block_id = blocks.id', 'join_type' => 'inner', 'join_select' => array('blocks.encrypted_code as block_encrypted_code', 'blocks.name as block_name, blocks.page_number, blocks.description'), 'join_condition' => array('blocks.status' => '1'));
                        $join_logic[2] = array('first_table' => 'surveys', 'join_on' => 'surveys.id = blocks.survey_id', 'join_type' => 'inner', 'join_select' => array('surveys.encrypted_code as survey_encrypted_code'), 'join_condition' => array('surveys.encrypted_code' => $survey_code));
                        $join_logic[3] = array('first_table' => 'question_details', 'join_on' => 'question_details.question_id = questions.id', 'join_type' => 'inner', 'join_select' => array('question_details.question_id', 'question_details.is_mandatory, question_details.question_sum, question_details.upper_limit, question_details.lower_limit, question_details.limit_interval, question_details.question_code, question_details.is_other_text, question_details.other_text_name, question_details.data_label'), 'join_condition' => array('question_details.question_code' => $question_code));
                        $join_logic[4] = array('first_table' => 'options', 'join_on' => 'options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('options.encrypted_code as option_encrypted_code', 'options.id as option_id', 'options.name as option_name', 'options.lower_label_text', 'options.upper_label_text'));
                        $join_logic[5] = array('first_table' => 'matrix_options', 'join_on' => 'matrix_options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('matrix_options.encrypted_code as matrix_option_encrypted_code', 'matrix_options.name as matrix_option_name'));
                        ...
                        $pc_questions[$question_code] = $this->fetch($p_table = 'questions', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('blocks.block_order' => 'ASC', 'block_questions.question_order ' => 'ASC', 'options.option_order' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
                    }
                ...
                $cnt++;
                if ($cnt == count($blocks)) {
                    //echo $cnt . ' => ' . count($blocks) . ' TRUE <br />';
                    $data['responses'][$block_id] = $this->get_data_from_temp_table($survey_id, $block_id, $no_reply, $last_block = TRUE);
                } else {
                    //echo $cnt . ' => ' . count($blocks) . ' FALSE <br />';
                    $data['responses'][$block_id] = $this->get_data_from_temp_table($survey_id, $block_id, $no_reply, $last_block = FALSE);
                }
            }
    ...
    public function get_data_from_temp_table($survey_id = '0', $block_id = '0', $no_reply = TRUE, $last_block = FALSE) {
        ...
        $join_logic[0] = array('first_table' => 'block_questions', 'join_on' => 'block_questions.question_id = questions.id', 'join_type' => 'inner', 'join_condition' => array('block_questions.status' => '1'));
        $join_logic[1] = array('first_table' => 'blocks', 'join_on' => 'block_questions.block_id = blocks.id', 'join_type' => 'inner', 'join_select' => array('blocks.encrypted_code as block_encrypted_code', 'blocks.name as block_name, blocks.page_number, blocks.description'), 'join_condition' => array('blocks.status' => '1'));
        $join_logic[2] = array('first_table' => 'surveys', 'join_on' => 'surveys.id = blocks.survey_id', 'join_type' => 'inner', 'join_select' => array('surveys.encrypted_code as survey_encrypted_code'), 'join_condition' => array('surveys.id' => $survey_id));
        $join_logic[3] = array('first_table' => 'question_details', 'join_on' => 'question_details.question_id = questions.id', 'join_type' => 'inner', 'join_select' => array('question_details.question_id', 'question_details.is_mandatory, question_details.question_sum, question_details.upper_limit, question_details.lower_limit, question_details.limit_interval, question_details.question_code, question_details.is_other_text, question_details.is_none_of_the_above, question_details.other_text_weightage, question_details.none_of_the_above_weightage'));
        ...
        $join_logic[5] = array('first_table' => 'matrix_options', 'join_on' => 'matrix_options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('matrix_options.encrypted_code as matrix_option_encrypted_code', 'matrix_options.name as matrix_option_name', 'matrix_options.option_weightage as matrix_option_weightage', 'matrix_options.id as matrix_option_id'));
        $join_logic[6] = array('first_table' => 'question_types', 'join_on' => 'question_types.id = question_details.question_type_id', 'join_type' => 'inner', 'join_select' => array('question_types.encrypted_code as question_type_encrypted_code, question_types.name as question_type'), 'join_condition' => array('question_types.status' => '1'));
        $pc_questions = $this->fetch($p_table = 'questions', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('CAST(options.option_order AS DECIMAL)' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
        ...
        if (isset($raw_respondents)) {
            ...
            foreach ($raw_respondents as $rawp) {
                ...
                $cond_and = array();
                //$cond_and = array('respondent_id' => $rawp['respondent_id']);
                $cond_and = array('respondent_code' => $rawp['respondent_code']);
                $responses = $this->fetch($p_table = $response_table, $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
                if (isset($responses)) {
                    foreach ($responses as $response) {
                        ...
                        if (in_array($question_type_id, array('1'))) {
                            /* Dichotomous Question */
                            $data_to_save[$field] = $option_text[$option_id] . '~' . $option_weightage[$option_id];
                        }
                        if (in_array($question_type_id, array('2'))) {
                            /* Text Response Question */
                            $data_to_save[$field] = $text_response;
                        }
                        if (in_array($question_type_id, array('3'))) {
                            /* Multiple Choice Question */
                            $data_to_save[$field] = isset($option_text[$option_id]) ? $option_text[$option_id] . '~' . $option_weightage[$option_id] : ($other_text_response == 'NOTA' ? 'NOTA~' . $nota_weightage[$question_id] : $other_text_response . '~' . $other_weightage[$question_id]);
                        }
                        if (in_array($question_type_id, array('4'))) {
                            /* Single Choice Question */
                            $data_to_save[$field] = $other_text_response == '' ? $option_text[$option_id] . '~' . $option_weightage[$option_id] : ($other_text_response == 'NOTA' ? 'NOTA~' . $nota_weightage[$question_id] : $other_text_response . '~' . $other_weightage[$question_id]);
                        }
                        if (in_array($question_type_id, array('5'))) {
                            /* Date and Time Question */
                            $data_to_save[$field] = $text_response;
                        ...
                        if (in_array($question_type_id, array('8'))) {
                            /* Dropdown Question */
                            $data_to_save[$field] = isset($option_text[$option_id]) ? $option_text[$option_id] . '~' . $option_weightage[$option_id] : '0';
                        }
                        if (in_array($question_type_id, array('9'))) {
                            /* Number Question */
                            $data_to_save[$field] = $text_response;
                        ...
                        if (in_array($question_type_id, array('11'))) {
                            /* Single Choice Grid Question */
                            $field = 'field_' . $question_type_id . '_' . $question_id . '_' . $option_id . '_0_0_0';
                            $data_to_save[$field] = isset($matrix_option_text[$matrix_option_id]) ? $matrix_option_text[$matrix_option_id] . '~' . $matrix_option_weightage[$matrix_option_id] : (isset($option_text[$matrix_option_id]) ? $option_text[$matrix_option_id] : '0');
                        }
                        if (in_array($question_type_id, array('12'))) {
                            /* Multiple Choice Grid Question */
                            $data_to_save[$field] = isset($matrix_option_text[$matrix_option_id]) ? $matrix_option_text[$matrix_option_id] . '~' . $matrix_option_weightage[$matrix_option_id] : (isset($option_text[$matrix_option_id]) ? $option_text[$matrix_option_id] : '0');
                        ...
                        if (in_array($question_type_id, array('24'))) {
                            /* Image Upload Question */
                            $data_to_save[$field] = file_exists("./assets/uploads/survey/" . $file_name) ? '<a target="_blank" href="' . site_url("assets/uploads/survey/" . $file_name) . '">Image</a>' : "<span class=\"text-danger font-italic\">No file uploaded</span>";
                        }
                        if (in_array($question_type_id, array('25'))) {
                            /* Bipolar Matrix Question */
                            $data_to_save[$field] = isset($matrix_option_text[$matrix_option_id]) ? $matrix_option_text[$matrix_option_id] . '~' . $matrix_option_weightage[$matrix_option_id] : (isset($option_text[$matrix_option_id]) ? $option_text[$matrix_option_id] : '0');
                    ...
                    $data_to_save['device_id'] = $rawp['device_id'];
                    $data_to_save['device_name'] = $rawp['device_name'];
                    $data_to_save['longitude'] = $rawp['longitude'] <> "" && $rawp['lattitude'] <> "" ? '<a target="_blank" href="' . site_url('siteadmin/responses/response_location/' . $rawp['lattitude'] . '/' . $rawp['longitude']) . '">' . $rawp['longitude'] . '</a>' : '';
                    $data_to_save['latitude'] = $rawp['longitude'] <> "" && $rawp['lattitude'] <> "" ? '<a target="_blank" href="' . site_url('siteadmin/responses/response_location/' . $rawp['lattitude'] . '/' . $rawp['longitude']) . '">' . $rawp['lattitude'] . '</a>' : '';
                    $data_to_save['api_level'] = $rawp['api_level'];
                    $data_to_save['app_version'] = $rawp['app_version'];

                    $data_to_save['unique_code'] = strtoupper(substr($rawp['unique_code'], 0, 10));
                    $data_to_save['respondent_id'] = $rawp['respondent_id'];
                    $data_to_save['respondent_code'] = $respondent_code;
                    $data_to_save['device_type'] = $rawp['device_type'];
                    $data_to_save['started_on'] = date('h:i A d M, Y', strtotime($rawp['started_on']));
                    $data_to_save['ended_on'] = $rawp['ended_on'] <> "0000-00-00 00:00:00" ? date('h:i A d M, Y', strtotime($rawp['ended_on'])) : "";
                    ...
                    $data_to_save['duration'] = $rawp['ended_on'] <> "0000-00-00 00:00:00" ? $interval->format('%H Hrs | %i Mins | %s Sec') : "";
                    $data_to_save['ip_address'] = $rawp['ip_address'];
                    $data_to_save['tracking_code'] = $rawp['tracking_code'];

                    $data_to_save['accept_status'] = $rawp['status'] == 1 ? '<span class="text-accept-' . $respondent_code . '">Accepted</span>' : '<a title="Accepted" class="accept-response accept-' . $respondent_code . '" data-respondent-code="' . $respondent_code . '"><i class="fas fa-check mr-2 teal-text" aria-hidden="true"></i></a>';
                    $data_to_save['reject_status'] = $rawp['status'] == 5 ? '<span class="text-reject-' . $respondent_code . '">Rejected</span>' : '<a title="Rejected" class="reject-response reject-' . $respondent_code . '" data-respondent-code="' . $respondent_code . '"><i class="fas fa-trash mr-2 text-danger" aria-hidden="true"></i></a>';
                    $data_to_save['flag_status'] = $rawp['status'] == 2 ? '<span class="text-flag-' . $respondent_code . '">Flagged</span>' : '<a title="Note" class="flag-response flag-' . $respondent_code . '" data-respondent-code="' . $respondent_code . '"><i class="fas fa-check mr-2 text-warning" aria-hidden="true"></i></a>';
                    //$data_to_save['note'] = in_array($rawp['status'], array(2, 5)) ? '<span class="flag-note-' . $respondent_code . '">' . $rawp['note'] . '</span>' : '<span class="flag-note"></span>';
                    $data_to_save['note'] = in_array($rawp['status'], array(2, 5)) ? (($rawp['note'] <> NULL) ? $rawp['note'] : '<span class="flag-note-' . $respondent_code . '"></span>') : '<span class="flag-note-' . $respondent_code . '"></span>';
                    $data_to_save['enumerator'] = $rawp['enumerator_name'];
                    $data_to_save['survey_status'] = $rawp['status'];
                    $data_to_save['team'] = $rawp['team_code'];
                    
                    $resp = $this->save($p_table = $temp_table, $p_key = 'id', $data_array = $data_to_save, $cond = NULL, $debug = FALSE);
                    if ($resp && $last_block == TRUE) {
                        $data_array = array();
                        $data_array['respondent_id'] = $rawp['respondent_id'];
                        $data_array['is_copied_to_temp_table'] = '1';
                        $this->update($p_table = 'survey_respondents', $p_key = 'respondent_id', $data_array, $cond = NULL, $debug = FALSE);
                    }
                }
            }
        } else {
        ...
        if ($no_reply == FALSE) {
            $data = $this->fetch($p_table = $temp_table, $p_key = 'id', $cond_and = array(), $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
            return $data;
        } else {
            return TRUE;
        }
    ...
    public function generate_temp_roster_table($by_force = FALSE, $survey_code = '0', $survey_id = '0', $no_reply = TRUE) {
        if ($survey_code <> '0' && $survey_id <> '0') {
            ...
            $cond_and = array();
            $join_logic = array();
            $cond_and = array('survey_id' => $survey_id, 'is_roster' => '1');
            $blocks = $this->fetch($p_table = 'blocks', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('blocks.block_order' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
            $cnt = 0;
            foreach ($blocks as $block_row) {
                $block_id = $block_row['id'];
                ...
                $join_logic = array();
                $join_logic[0] = array('first_table' => 'block_questions', 'join_on' => 'block_questions.question_id = questions.id', 'join_type' => 'inner', 'join_select' => array('block_questions.block_id', 'block_questions.question_id'), 'join_condition' => array('block_questions.status' => '1', 'block_questions.block_id' => $block_id));
                $join_logic[1] = array('first_table' => 'blocks', 'join_on' => 'block_questions.block_id = blocks.id', 'join_type' => 'inner', 'join_select' => array('blocks.name as block_name'), 'join_condition' => array('blocks.is_roster' => '1', 'blocks.status' => '1'));
                $join_logic[2] = array('first_table' => 'surveys', 'join_on' => 'surveys.id = blocks.survey_id', 'join_type' => 'inner', 'join_select' => array('surveys.encrypted_code as survey_encrypted_code'), 'join_condition' => array('surveys.encrypted_code' => $survey_code));
                $join_logic[3] = array('first_table' => 'question_details', 'join_on' => 'question_details.question_id = questions.id', 'join_type' => 'inner', 'join_select' => array('question_details.question_type_id', 'question_details.question_sum, question_details.is_other_text, question_details.other_text_name, question_details.is_none_of_the_above, question_details.other_text_name, question_details.data_label, question_details.question_code'));
                $join_logic[4] = array('first_table' => 'options', 'join_on' => 'options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('options.id as option_id', 'options.name as option_name'));
                $join_logic[5] = array('first_table' => 'matrix_options', 'join_on' => 'matrix_options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('matrix_options.id as matrix_option_id', 'matrix_options.name as matrix_option_name'));
                ...
                if ($cnt == count($blocks)) {
                    //echo $cnt . ' => ' . count($blocks) . ' TRUE <br />';
                    $data['responses'][$block_id] = $this->get_data_from_temp_roster_table($survey_id, $block_id, $no_reply, $last_block = TRUE);
                } else {
                    //echo $cnt . ' => ' . count($blocks) . ' FALSE <br />';
                    $data['responses'][$block_id] = $this->get_data_from_temp_roster_table($survey_id, $block_id, $no_reply, $last_block = FALSE);
                }
            }
    ...
    public function get_data_from_temp_roster_table($survey_id = '0', $block_id = '0', $no_reply = TRUE, $last_block = FALSE) {
        /* INSERT DATA IF LEFT STARTS */
        $join_logic = array();
        ...
        if (isset($raw_respondents)) {
            ...
            foreach ($raw_respondents as $rawp) {
                ...
                //$cond_and = array('respondent_id' => $rawp['respondent_id']);
                $cond_and = array();
                $join_logic[0] = array('first_table' => 'survey_roster', 'join_on' => 'survey_roster.roster_id = '.$response_table.'.roster_id', 'join_type' => 'inner', 'join_select' => array('survey_roster.roster_id, survey_roster.parent_roster_id'), 'join_condition' => array('survey_roster.status' => '1', 'survey_roster.roster_id' => $rawp['roster_id']));
                $join_logic[1] = array('first_table' => 'block_questions', 'join_on' => 'block_questions.question_id = '.$response_table.'.question_id', 'join_type' => 'inner', 'join_select' => array('block_questions.block_id'), 'join_condition' => array('block_questions.status' => '1', 'block_questions.block_id' => $block_id));
                $responses = $this->fetch($p_table = $response_table, $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
                if (isset($responses)) {
                    foreach ($responses as $response) {
                        $question_type_id = $response['question_type_id'];
                        $question_id = $response['question_id'];
                    ...
                    $data_to_save['unique_code'] = strtoupper(substr($rawp['unique_code'], 0, 10));
                    $data_to_save['respondent_id'] = $rawp['respondent_id'];
                    $data_to_save['roster_id'] = $roster_id;
                    $data_to_save['parent_roster_id'] = $parent_roster_id;
                    $data_to_save['respondent_code'] = $respondent_code;
                    $data_to_save['device_type'] = $rawp['device_type'];
                    $data_to_save['started_on'] = date('h:i A d M, Y', strtotime($rawp['started_on']));
                    $data_to_save['ended_on'] = $rawp['ended_on'] <> "0000-00-00 00:00:00" ? date('h:i A d M, Y', strtotime($rawp['ended_on'])) : "";
                    ...
                    if ($resp && $last_block == TRUE) {
                        $data_array = array();
                        $data_array['respondent_id'] = $rawp['respondent_id'];
                        $data_array['is_copied_to_temp_roster_table'] = '1';
                        $this->update($p_table = 'survey_respondents', $p_key = 'respondent_id', $data_array, $cond = NULL, $debug = FALSE);
                    }
                }
            }
