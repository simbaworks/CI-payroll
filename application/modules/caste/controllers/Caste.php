...
class Caste extends MY_Controller {
    ...
    public function index() {
        header('Location:' . site_url('caste/lists'));
        exit();
    }
    ...
    public function lists() {        
        ...
        $pagiConfig['total_rows'] = $this->Common->fetch($p_table = 'caste', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = TRUE, $return_object = FALSE, $debug = FALSE);
        ...
        $this->data['results'] = $this->Common->fetch($p_table = 'caste', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'DESC'), $limit = $pagiConfig['per_page'], $offset = $this->data['offset'], $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
        ...
        $this->data['data_table'] = $this->load->view('caste/ajax/lists', $this->data, TRUE);
    ...
    public function ajax_insta_edit_content(){  
        ...
        $results = $this->Common->fetch($p_table = 'caste', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'ASC'), $limit = NULL, $offset = NULL, $group_by = array(), $row_only = TRUE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
        ...
        if(isset($results)) {
            ...
            $this->Common->update($p_table = 'caste', $p_key = 'id', $data_array = $data_to_save, $cond = NULL, $debug = FALSE);
            ...
            $system_log_array['action'] = 'Modified an existing record in caste | ' . $action_string;
    ...
    public function ajax_insta_status_update(){  
        ...
        if(isset($results)) {
            ...
            $system_log_array['method_name'] = $this->data['method'];
            $system_log_array['row_id'] = $data_id;
            $system_log_array['action'] = 'Changed status of an existing record in caste';
            $system_log_array['time'] = date('Y-m-d H:i:s');
            $system_log_array['ip_address'] = $this->input->ip_address();
            $system_log_array['user_browser'] = $this->get_browser();
            $temp = $this->Common->save($p_table = 'system_log', $p_key = 'id', $data_array = $system_log_array, $cond = array(), $debug = FALSE);
    ...
    public function ajax_load_add_form(){
        ...
        $html = $this->load->view('caste/ajax/ajax_add_form', $this->data, TRUE);

        $response['html'] = $html;
        $response['title'] = 'Add new Caste';
        echo json_encode($response);
    }

    public function ajax_save_add_form_data(){  
        $caste_name = isset($this->data['resp']['caste_name']) ? base64_encode($this->data['resp']['caste_name']) : NULL;
        ...
        $data_to_save['caste_name'] = $caste_name;
        ...
        $row_id = $this->Common->save($p_table = 'caste', $p_key = 'id', $data_array = $data_to_save, $cond = NULL, $debug = FALSE);
        if(isset($row_id)){
            /* ------------------------------------- SYSTEM LOG STARTS ------------------------------------- */
            ...
            $system_log_array['action'] = 'Added a new record in castes | ' . $action_string;
            $system_log_array['time'] = date('Y-m-d H:i:s');
            $system_log_array['ip_address'] = $this->input->ip_address();
            $system_log_array['user_browser'] = $this->get_browser();
            $temp = $this->Common->save($p_table = 'system_log', $p_key = 'id', $data_array = $system_log_array, $cond = array(), $debug = FALSE);
    ...
    function ajax_refresh_page($search_text = NULL) {  
        ...
        if($search_text !== NULL){
            $cond_custom = ' AND FROM_BASE64(`caste_name`) LIKE "%' . $search_text . '%"';
        }

        //pagination settings
        $pagiConfig['base_url'] = base_url($this->data['controller'] . '/lists');

        $pagiConfig['total_rows'] = $this->Common->fetch($p_table = 'caste', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = TRUE, $return_object = FALSE, $debug = FALSE);
        $pagiConfig['per_page'] = $this->data["per_page"];
        $pagiConfig["uri_segment"] = 3;
        $choice = $pagiConfig["total_rows"] / $pagiConfig["per_page"];
        $pagiConfig["num_links"] = floor($choice) < 5 ? floor($choice) : 5; //floor( $choice );
        ...
        $this->data['results'] = $this->Common->fetch($p_table = 'caste', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'DESC'), $limit = $pagiConfig['per_page'], $offset = $this->data['offset'], $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
        ...
        $html = $this->load->view('caste/ajax/ajax_lists', $this->data, TRUE);
    ...
    public function ajax_auto_update(){  
        $cond_and = array();
        $cond_and = array('status <>' => '5');
        $results = $this->Common->fetch($p_table = 'caste', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'ASC'), $limit = NULL, $offset = NULL, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
        ...
        if(isset($results)) {
            foreach($results as $value){
                ...
                $data_to_save['unique_code'] = $this->_random_password(32);
                $data_to_save['encrypted_code'] = $this->_random_password(32);
                $data_to_save['caste_name'] = base64_encode($value['caste_name']);
                $data_to_save['status'] = '1';
                $data_to_save['created'] = date("Y-m-d H:i:s");
                $data_to_save['created_by'] = $this->session->userdata($this->data['sess_code'] . 'user_id'); 
