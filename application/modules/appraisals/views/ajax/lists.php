          		...
          		<?php if(isset($results)){$i = $offset + 1;?>
          			<?php foreach ($results as $value) {?>
            			<tr>
							...
							<td>
								<?php echo isset($employees[$value['reporting_off_id']])? base64_decode($employees[$value['reporting_off_id']]) : '';?>
							</td>
							<td>
								<?php echo isset($employees[$value['reviewing_off_id']])? base64_decode($employees[$value['reviewing_off_id']]) : '';?>
							</td>
							<td>
								<?php echo isset($value['marks_obtained'])? $value['marks_obtained'] : '';?>
							</td>
							<td>
								<?php if (isset($value['supporting_document']) && file_exists('./assets/uploads/appraisals/' . $value['supporting_document'])) { ?>
									...
									<a class="btn btn-outline-info btn-sm" href="<?php echo site_url('assets/uploads/appraisals/' . $value['supporting_document']);?>" target="_blank" title="View supporting document"><?php echo $icon;?></a>
								<?php }?>
							</td>
            			</tr>
            		<?php $i++;}?>
