...
class Commonmodel extends CI_Model {
    ...
    public function generate_temp_table($by_force = FALSE, $survey_code = '0', $survey_id = '0', $no_reply = TRUE) {
        if ($survey_code <> '0' && $survey_id <> '0') {
            ...
            $join_logic[3] = array('first_table' => 'question_details', 'join_on' => 'question_details.question_id = questions.id', 'join_type' => 'inner', 'join_select' => array('question_details.question_id, question_details.question_type_id, question_details.is_other_text, question_details.other_text_name, question_details.is_none_of_the_above, question_details.data_label, question_details.question_code, question_details.is_other_text, question_details.other_text_name, question_details.data_label, question_details.other_text_weightage, question_details.none_of_the_above_weightage'));
            ...
            $all_questions = $this->fetch($p_table = 'questions', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('CAST(blocks.block_order AS DECIMAL)' => 'ASC', 'CAST(block_questions.question_order AS DECIMAL)' => 'ASC', 'CAST(options.option_order AS DECIMAL)' => 'ASC', 'CAST(matrix_options.option_order AS DECIMAL)' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
            */
            ...
            $join_logic[1] = array('first_table' => 'blocks', 'join_on' => 'block_questions.block_id = blocks.id', 'join_type' => 'inner', 'join_select' => array('blocks.encrypted_code as block_encrypted_code, blocks.name as block_name, blocks.page_number, blocks.description, blocks.is_roster'), 'join_condition' => array('blocks.status' => '1'));
            $join_logic[2] = array('first_table' => 'surveys', 'join_on' => 'surveys.id = blocks.survey_id', 'join_type' => 'inner', 'join_select' => array('surveys.encrypted_code as survey_encrypted_code'), 'join_condition' => array('surveys.encrypted_code' => $survey_code));
            $join_logic[3] = array('first_table' => 'options', 'join_on' => 'options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('options.encrypted_code as option_encrypted_code, options.id as option_id, options.name as option_name, options.lower_label_text, options.upper_label_text, options.option_weightage'));
            $join_logic[4] = array('first_table' => 'matrix_options', 'join_on' => 'matrix_options.question_id = questions.id', 'join_type' => 'left', 'join_select' => array('matrix_options.id as matrix_option_id', 'matrix_options.name as matrix_option_name, matrix_options.option_weightage as matrix_option_weightage'));
            $all_questions = $this->fetch($p_table = 'questions', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('CAST(blocks.block_order AS DECIMAL)' => 'ASC', 'CAST(block_questions.question_order AS DECIMAL)' => 'ASC', 'CAST(options.option_order AS DECIMAL)' => 'ASC', 'CAST(matrix_options.option_order AS DECIMAL)' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
            
            $question_id_array = [];
            if(isset($all_questions)){
                foreach($all_questions as $aq){
                    $question_id_array[$aq['id']] = $aq['id'];
                }
            }
            ...
            $cond_and = array('status' => '1');
            $cond_in = array();
            $cond_in = array('question_id' => $question_id_array);
            $join_logic = array();
            $join_logic[0] = array('first_table' => 'question_types', 'join_on' => 'question_types.id = question_details.question_type_id', 'join_type' => 'inner', 'join_select' => array('question_types.encrypted_code as question_type_encrypted_code, question_types.name as question_type'), 'join_condition' => array('question_types.status' => '1'));
            $question_details = $this->fetch($p_table = 'question_details', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in, $cond_custom = NULL, $select_fields = array('question_id', 'question_type_id', 'is_other_text', 'other_text_name', 'is_none_of_the_above', 'data_label', 'question_code', 'is_other_text', 'other_text_name', 'data_label', 'other_text_weightage', 'none_of_the_above_weightage'), $join_logic, $order_by = array('id'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
            
            $all_questions_new = array();
            if(isset($all_questions)){
                foreach($all_questions as $aq){
                    foreach($question_details as $qd){
                        if($aq['id'] == $qd['question_id']){
                            $aq1['question_id'] = $qd['question_id'];
                            $aq1['question_type_id'] = $qd['question_type_id'];
                            $aq1['is_other_text'] = $qd['is_other_text'];
                            $aq1['other_text_name'] = $qd['other_text_name'];
                            $aq1['is_none_of_the_above'] = $qd['is_none_of_the_above'];
                            $aq1['question_code'] = $qd['question_code'];
                            $aq1['is_other_text'] = $qd['is_other_text'];
                            $aq1['other_text_name'] = $qd['other_text_name'];
                            $aq1['data_label'] = $qd['data_label'];
                            $aq1['other_text_weightage'] = $qd['other_text_weightage'];
                            $aq1['other_text_weightage'] = $qd['other_text_weightage'];
                            $aq1['none_of_the_above_weightage'] = $qd['none_of_the_above_weightage'];
                            $aq1['question_type_encrypted_code'] = $qd['question_type_encrypted_code'];
                            $aq1['question_type'] = $qd['question_type'];
                            $aq = array_merge($aq,$aq1);
                        }
                    }
                    array_push($all_questions_new, $aq);
                }
            }
            $all_questions = $all_questions_new;
            ...
            if(isset($blocks)){
                foreach ($blocks as $block_row) {
                    ...
                    if (isset($all_questions)) {
                        foreach ($all_questions as $question) {
                            if($question['block_id'] == $block_id){
                                ...
                                $results[$question['block_id']]['questions'][$question['question_id']]['question_id'] = $question['question_id'];
                    ...
                    if ($by_force == TRUE || !$this->db->table_exists($table_report)) {
                        ...
                        foreach ($results as $block_id => $details) {
                            foreach ($details['questions'] as $question_id => $question_details) {
                                ...
                                if (in_array($question_type_id, array('1'))) {
                                    ...
                                    if (isset($question_details['options'])) {
                                        foreach ($question_details['options'] as $option_id => $option_details) {
                                            ...
                                            $data_row_1[$field[$i]] = $question_type_id . '_' . $question_id . '_' . $option_id;
                                ...
                                if (in_array($question_type_id, array('2'))) {
                                    ...
                                    $field[$i] = $static_part . $question_type_id . '_' . $question_id . '_' . $option_id . '_' . $matrix_option_id . '_' . $row_question_id . '_' . $column_question_id;
                                    $data_row_1[$field[$i]] = $question_type_id . '_' . $question_id;
                                    //$data_row_1[$field[$i]] = $data_row_1[$field[$i]] . "|+|" . $data_label_text;
                                    $i++;
                                }
                                            ...
                                            if (strpos($option_text, '[[') === false) {
                                                ...
                                                foreach ($question_details['matrix_options'] as $matrix_option_id => $matrix_option_details) {
                                                    ...
                                                    if (strpos($matrix_option_text, '[[') === false) {
                                                        ...
                                                        $field[$i] = $static_part . $question_type_id . '_' . $question_id . '_' . $option_id . '_' . $matrix_option_id . '_' . $row_question_id . '_' . $column_question_id;
                                                        $data_row_1[$field[$i]] = $question_type_id . '_' . $question_id . '_' . $option_id . '_' . $matrix_option_id;
                                                        //$data_row_1[$field[$i]] = $data_row_1[$field[$i]] . "|+|" . $data_label_text;
                                                        $i++;
                                                        $section_count++;
                                                        ...
                                                        foreach ($match[1] as $dynamic_row) {
                                                            ...
                                                            foreach ($pc_question_details[$dynamic_row] as $dynamic_matrix_option_id => $pc_details) {
                                                                ...
                                                                $data_row_1[$field[$i]] = $question_type_id . '_' . $question_id . '_' . $option_id . '_' . $dynamic_matrix_option_id;
                                                                //$data_row_1[$field[$i]] = $data_row_1[$field[$i]] . "|+|" . $data_label_text;
                                                                $i++;
                                                                $section_count++;
                                                                ...
                                                                if ($pc_details['is_other_text'] == '1' && count($pc_question_details[$dynamic_row]) == $section_count) {
                                                                    ...
                                                                    $data_row_1[$field[$i]] = $question_type_id . '_' . $question_id . '_' . $option_id . '_' . $pc_details['other_text_name'];
                                                                    //$data_row_1[$field[$i]] = $data_row_1[$field[$i]] . "|+|" . $data_label_text;
                                                                    $i++;
                                                                }
                                                            }
                                                        ...
                                                        foreach ($question_details['matrix_options'] as $matrix_option_id => $matrix_option_details) {
                                                            ...
                                                            if (strpos($matrix_option_text, '[[') === false) {
                                                                ...
                                                                if ($is_other_text == '1') {
                                                                    ...
                                                                    $data_row_1[$field[$i]] = $question_type_id . '_' . $question_id . '_' . $other_text_name . '_' . $matrix_option_id;
                                                                    //$data_row_1[$field[$i]] = $data_row_1[$field[$i]] . "|+|" . $data_label_text;
                                                                    $i++;
                                                                }
                                                                /* IF PARENT CHILD COLUMN IS NOT PRESENT ENDS */
                        ...
                        $attributes = array('ENGINE' => 'InnoDB', 'ROW_FORMAT' => 'COMPRESSED');
    ...
    public function get_temp_table_data($survey_id = '0', $blocks = array(), $no_reply = TRUE, $pc_questions = array(), $raw_respondents = array()) {
        ...
        if (isset($pc_questions)) {
            foreach ($pc_questions as $qs_row) {
                ...
                $nota_weightage[$qs_row['question_id']] = $qs_row['none_of_the_above_weightage'];
                $other_weightage[$qs_row['question_id']] = $qs_row['other_text_weightage'];
            }
        }
        $response_table = 'responses_' . $survey_id;
        //$join_logic = array();
        //$join_logic[0] = array('first_table' => 'survey_respondents', 'join_on' => 'survey_respondents.respondent_code = '.$response_table.'.respondent_code', 'join_type' => 'inner', 'join_condition' => array('is_copied_to_temp_table' => '0'));
        //$response_array = $this->fetch($p_table = $response_table, $p_key = 'id', $cond_and = array(), $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic, $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = 1);
        $sql = "SELECT `".$response_table."`.* FROM `".$response_table."` WHERE `".$response_table."`.`respondent_code` IN (SELECT `survey_respondents`.`respondent_code` FROM `survey_respondents` WHERE `survey_respondents`.`is_copied_to_temp_table` = '0' AND `survey_respondents`.`survey_id` = '".$survey_id."') ORDER BY `id` ASC";
        $raw_response_array = $this->db->query($sql);
        $response_array = $raw_response_array->result_array();
        ...
        foreach ($blocks as $block_row) {
            ...
            $temp_table_data = $this->fetch($p_table = $temp_table, $p_key = 'id', $cond_and = array(), $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*'), $join_logic = array(), $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
            if(isset($temp_table_data)){
                $trow = 0;
                foreach($temp_table_data as $k => $ttd){
                    if($trow == 0){
                        continue;
                    }
                    $temp_data[$ttd['respondent_code']] = $ttd['respondent_code'];
                    $trow++;
                }
            }
            ...
            if (isset($raw_respondents)) {
                ...
                foreach ($raw_respondents as $rawp) {
                    if(in_array($rawp['respondent_code'], $temp_data)){
                        continue;
                    }
                    if (isset($response_row[$rawp['respondent_code']])) {
                        ...
                        $data_to_save[$rec_count]['synced_on'] = $rawp['created'] <> "0000-00-00 00:00:00" ? date('h:i A d M, Y', strtotime($rawp['created'])) : "";
                        ...
                        $minutes += $interval->i;
                        $data_to_save[$rec_count]['duration'] = $rawp['ended_on'] <> "0000-00-00 00:00:00" ? $minutes : "";
        ...
        if ($survey_code <> '0' && $survey_id <> '0') {
            ...
            */
            ...
            if(isset($blocks)){
                ...
                //echo '<br />----------------------------------------<br />'; echo date('Y-m-d H:i:s'); echo '<br />----------------------------------------<br />'; //exit();
                foreach ($blocks as $block_row) {
                    $block_ids[] = $block_row['block_id'];
                }
                //echo '<pre>'; print_r($block_ids); echo '</pre>';
                ...
                //$join_logic[0] = array('first_table' => 'block_questions', 'join_on' => 'block_questions.question_id = '.$response_table.'.question_id', 'join_type' => 'inner', 'join_select' => array('block_questions.block_id'), 'join_condition' => array('block_questions.status' => '1'), 'join_in' => array('block_questions.block_id' => $block_ids));
                //$join_logic[0] = array('first_table' => 'survey_respondents', 'join_on' => 'survey_respondents.respondent_code = '.$response_table.'.respondent_code', 'join_type' => 'inner', 'join_condition' => array('survey_respondents.is_copied_to_temp_roster_table' => '0'));
                //$response_array = $this->fetch($p_table = $response_table, $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*, (SELECT DISTINCT(`parent_roster_id`) FROM `survey_roster` WHERE `survey_roster`.`roster_id` = `'.$response_table.'`.`roster_id`) AS `parent_roster_id`'), $join_logic, $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = 1);
                //exit();
                //$join_logic[0] = array('first_table' => 'survey_roster', 'join_on' => 'survey_roster.roster_id = '.$response_table.'.roster_id', 'join_type' => 'inner', 'join_select' => array('survey_roster.roster_id, survey_roster.parent_roster_id'), 'join_condition' => array('survey_roster.status' => '1'));
                //$join_logic[0] = array('first_table' => 'block_questions', 'join_on' => 'block_questions.question_id = '.$response_table.'.question_id', 'join_type' => 'inner', 'join_select' => array('block_questions.block_id'), 'join_condition' => array('block_questions.status' => '1'), 'join_in' => array('block_questions.block_id' => $block_ids));
                //////$join_logic[0] = array('first_table' => 'survey_respondents', 'join_on' => 'survey_respondents.respondent_code = '.$response_table.'.respondent_code', 'join_type' => 'inner', 'join_condition' => array('survey_respondents.is_copied_to_temp_roster_table' => '0'));
                //////$response_array = $this->fetch($p_table = $response_table, $p_key = 'id', $cond_and, $cond_or = array(), $cond_in = array(), $cond_custom = NULL, $select_fields = array('*, (SELECT DISTINCT(`parent_roster_id`) FROM `survey_roster` WHERE `survey_roster`.`roster_id` = `'.$response_table.'`.`roster_id`) AS `parent_roster_id`'), $join_logic, $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = 1);
                $sql = "SELECT `".$response_table."`.*, (SELECT DISTINCT(`parent_roster_id`) FROM `survey_roster` WHERE `survey_roster`.`roster_id` = `".$response_table."`.`roster_id`) AS `parent_roster_id` FROM `".$response_table."` WHERE `".$response_table."`.`respondent_code` IN (SELECT `survey_respondents`.`respondent_code` FROM `survey_respondents` WHERE `survey_respondents`.`is_copied_to_temp_roster_table` = '0' AND `survey_respondents`.`survey_id` = '".$survey_id."') ORDER BY `id` ASC";
                $raw_response_array = $this->db->query($sql);
                $response_array = $raw_response_array->result_array();
                //exit();
    ...
    public function get_temp_roster_table_data($survey_id = '0', $blocks = array(), $no_reply = TRUE, $pc_questions = array(), $response_array = array()) {
        ...
        if (isset($pc_questions)) {
            foreach ($pc_questions as $qs_row) {
                $block_by_question_id[$qs_row['question_id']] = $qs_row['block_id'];
            }
        }
        if(isset($response_array)){
            foreach($response_array as $response_data){
                $response_row[$response_data['roster_id']][$block_by_question_id[$response_data['question_id']]][] = $response_data;
        ...
        $raw_respondents = $this->fetch($p_table = 'survey_respondents', $p_key = 'id', $cond_and, $cond_or = array(), $cond_in, $cond_custom = NULL, $select_fields = array('*, (SELECT roster_id FROM survey_roster WHERE survey_roster.respondent_code = survey_respondents.respondent_code AND survey_roster.survey_id = \''.$survey_id.'\' ORDER BY id DESC LIMIT 1) AS roster_id'), $join_logic, $order_by = array('id' => 'ASC'), $limit = NULL, $offset = 0, $group_by = array(), $row_only = FALSE, $count_only = FALSE, $return_object = FALSE, $debug = FALSE);
        //echo '<pre>'; print_r($raw_respondents); exit();
        $enm_ids = array();
        if(isset($raw_respondents)){
            foreach($raw_respondents as $rr){
                $enm_ids[$rr['enumerator_id']] = $rr['enumerator_id'];
            }
            $sql = "SELECT id, name, (SELECT GROUP_CONCAT(team_enumerators.team_code) FROM team_enumerators WHERE team_enumerators.`enumerator_code` = enumerators.`encrypted_code`) AS team_code FROM enumerators WHERE id IN ('". implode(',', $enm_ids)."')";
            $raw_query = $this->db->query($sql);
            $enm_team_raw = $raw_query->result_array();
            
            if(isset($enm_team_raw)){
                foreach($enm_team_raw as $etr){
                    $enm_team[$etr['id']]['enumerator_name'] = $etr['name'];
                    $enm_team[$etr['id']]['team_code'] = $etr['team_code'];
                }
            }
        }
        ...
        foreach ($blocks as $block_row) {
            ...
            if(isset($temp_table_data)){
                ...
                foreach($temp_table_data as $k => $ttd){
                    ...
                    $temp_data[$ttd['roster_id']] = $ttd['roster_id'];
                    $trow++;
                }
            }
            $respondent_ids = array();
            ...
            if (isset($raw_respondents)) {
                /* COPY ROW FROM RESPONSE TABLE */
                foreach ($raw_respondents as $rawp) {
                    if(in_array($rawp['roster_id'], $temp_data)){
                        continue;
                    }
                    if (isset($response_row[$rawp['roster_id']][$block_id])) {
                        ...
                        //$data_to_save[$rec_count]['note'] = in_array($rawp['status'], array(2, 5)) ? '<span class="flag-note-' . $respondent_code . '">' . $rawp['note'] . '</span>' : '<span class="flag-note"></span>';
                        $data_to_save[$rec_count]['note'] = in_array($rawp['status'], array(2, 5)) ? (($rawp['note'] <> NULL) ? $rawp['note'] : '<span class="flag-note-' . $respondent_code . '"></span>') : '<span class="flag-note-' . $respondent_code . '"></span>';
                        $data_to_save[$rec_count]['enumerator'] = isset($enm_team[$rawp['enumerator_id']]['enumerator_name']) ? $enm_team[$rawp['enumerator_id']]['enumerator_name'] : '';
                        $data_to_save[$rec_count]['survey_status'] = $rawp['status'];
                        $data_to_save[$rec_count]['team'] = isset($enm_team[$rawp['enumerator_id']]['team_code']) ? $enm_team[$rawp['enumerator_id']]['team_code'] : '';
